        ...
        match constraint {
            ...
            ast::JoinConstraint::Using(distinct_names) => {
                ...
                for distinct_name in distinct_names.iter() {
                    ...
                    let Some((left_expr, left_columns)) = find_left_using_column(
                        left_tables,
                        distinct_name.as_str(),
                        natural,
                        has_right_or_full_join,
...
/// Build the left operand for one USING equality term.
///
...
fn find_left_using_column(
    ...
    has_right_or_full_join: bool,
) -> Result<Option<(Expr, SmallVec<[(TableInternalId, usize); 4]>)>> {
    ...
    Ok(Some((merge_columns(expressions), source_columns)))
}
