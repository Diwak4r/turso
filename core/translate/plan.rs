...
pub struct UnmatchedRightRowsPlan {
    ...
    /// Subqueries used by table-function arguments.
    ///
    /// SQLite plans these subqueries again for its one-table unmatched-right read.
    pub subqueries: Vec<NonFromClauseSubquery>,
    /// Table-function conditions that SQLite must check after `VFilter`.
    ///
    /// SQLite checks these conditions inside its one-table unmatched-right loop.
    /// The shared result subroutine excludes terms from the original outer join.
